...
back. The offer producer reopens either form. An inclusion starts a pending reward clock at
its `confirmed_block`; a readback only when `confirmed_last_update` is not older than the projection.
