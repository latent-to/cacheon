...
| Commissioned qualification incumbent differs from the durable evaluation stack | Refused before any lease, request, or GPU action; no candidate signal | Recommission from the current crowned stack: `chain-incumbent-authority` writes that stack and its retained bundle bytes for the next commission |
