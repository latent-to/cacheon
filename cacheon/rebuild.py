...
(``cacheon/patchers/``), selected via ``repo_python``. It must NOT execute
...
into ``cacheon/patchers/`` first; then a bundle's ``rebuild.json`` may select it by name.
...
meaningful sense. So the resolved script must live under ``cacheon/patchers/`` AND be a
...
def _repo_root() -> Path:
    ...
    happened to launch from. ``CACHEON_REPO_ROOT`` overrides for relocated deployments."""
    env = os.environ.get("CACHEON_REPO_ROOT")
    if env:
        return Path(env).resolve()
    # cacheon/rebuild.py -> cacheon/ -> repo root
...
def _run_python_script(
    ...
    ``CACHEON_BUNDLE_PATH`` is the patcher contract: every caller of
    ...
    validated nothing)."""
    old_argv = sys.argv
    old_bundle = os.environ.get("CACHEON_BUNDLE_PATH")
    old_phase = os.environ.get("CACHEON_REBUILD_PHASE")
    sys.argv = [str(script)]
    os.environ["CACHEON_BUNDLE_PATH"] = str(bundle)
    os.environ["CACHEON_REBUILD_PHASE"] = phase
    try:
        ...
        if old_bundle is None:
            ...
            os.environ["CACHEON_BUNDLE_PATH"] = old_bundle
        if old_phase is None:
            os.environ.pop("CACHEON_REBUILD_PHASE", None)
        else:
            os.environ["CACHEON_REBUILD_PHASE"] = old_phase
...
def _main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(prog="python -m cacheon.rebuild")
    parser.add_argument("--phase", choices=sorted(_REBUILD_PHASES), required=True)
    parser.add_argument("bundle")
    args = parser.parse_args(argv)
    if args.phase == "build" and os.environ.get("CACHEON_REBUILD_CONTAINER") != "1":
        raise RebuildError(
            "build subprocess entry requires the disposable rebuild container"
        )
    if args.phase == "load" and os.environ.get("CACHEON_ENGINE_WORKER") != "1":
        raise RebuildError("load subprocess entry requires an isolated engine worker")
    if args.phase == "all" and os.environ.get("CACHEON_REBUILD_DEVELOPMENT") != "1":
        raise RebuildError(
            "combined rebuild entry is development-only; set "
            "CACHEON_REBUILD_DEVELOPMENT=1 explicitly"
        )
