...
so the same code serves both runs of the two-launch eval:
    ...
    CACHEON_BUNDLE_PATH   path to the bundle (empty -> baseline)
    CACHEON_ACTIVE        "1" to load + enable the miner kernel, else baseline
"""
...
from cacheon._strict import truthy_flag
...
def _install_materialized_namespace() -> bool:
    ...
    root = _ENGINE_TREE
    release_digest = os.environ.get("CACHEON_RELEASE_DESCRIPTOR_DIGEST", "")
    release_verified = os.environ.get("CACHEON_RELEASE_VERIFIED", "")
    if (
        os.environ.get("CACHEON_ENGINE_WORKER") != "1"
        or os.environ.get("CACHEON_BUNDLE_PATH") != root
        or _DIGEST.fullmatch(os.environ.get("CACHEON_ENGINE_TREE_DIGEST", "")) is None
        or _DIGEST.fullmatch(os.environ.get("CACHEON_STACK_DIGEST", "")) is None
        or (
            release_required
            and (
...
def _release_abort(message: str) -> None:
    ...
    raise SystemExit(f"cacheon signed release refused: {message}")
...
def activate() -> None:
    ...
    (cacheon/seams.py) — the same table the bootstrap watch-list and the compat canary
    ...
    if not bundle or not _truthy(os.environ.get("CACHEON_ACTIVE")):
        if release_required:
...
def _load_bundle_into_registry(bundle: str) -> None:
    ...
    if any(op.setup for op in manifest.ops) and not _truthy(
        os.environ.get("CACHEON_FRAMEWORK_MODE")
    ):
        raise RuntimeError(
            "bundle declares setup() but CACHEON_FRAMEWORK_MODE is not armed; "
    ...
    prebuilt = _truthy(os.environ.get("CACHEON_PREBUILT_ARTIFACTS"))
    engine_worker = _truthy(os.environ.get("CACHEON_ENGINE_WORKER"))
    ...
    for op in manifest.ops:
        ...
        if not scan.ok:  # defense-in-depth: re-scan in the worker before load
            logger.warning("cacheon: skip %s, failed scan: %s", op.slot, scan.violations)
            continue
        ...
        if getattr(op, "setup", None):
            ...
            if setup_key not in setup_done:
                ...
                logger.info("cacheon: ran setup() for %s", op.slot)
