...
from cacheon.cuda_cubin import CudaCubinError, _require_elf_cubin
...
from cacheon.patchers.build_cute_aot import (
...
def _log(message: str) -> None:
    print(f"[cacheon.build_cute_cubin] {message}", flush=True)
...
def build_cute_cubin_stage(bundle: Path, *, stage: Path) -> str:
    ...
    build_spec = _digest(
        os.environ.get("CACHEON_NATIVE_BUILD_SPEC_DIGEST", "").strip(),
        field="native build spec digest",
    )
    tree_digest = _digest(
        os.environ.get("CACHEON_ENGINE_TREE_DIGEST", "").strip(),
        field="engine tree digest",
    )
    target_architecture = os.environ.get("CACHEON_TARGET_GPU_ARCH", "").strip()
    ...
    timeout_raw = os.environ.get("CACHEON_NATIVE_COMPILE_TIMEOUT_S", "").strip()
    ...
    tmp_root_raw = os.environ.get("CACHEON_BUILD_TMPDIR", "").strip()
    ...
    private_root = Path(tempfile.mkdtemp(prefix="cacheon-cute-cubin-", dir=tmp_root))
    try:
...
def main() -> None:
    phase = _phase()
    bundle = _absolute_directory_env("CACHEON_BUNDLE_PATH")
    if phase == "build":
        build_cute_cubin_stage(
            bundle,
            stage=_absolute_directory_env("CACHEON_NATIVE_ARTIFACT_STAGE"),
        )
        return
    if phase == "load":
        validate_cute_cubin_publication(
            bundle,
            root=_absolute_directory_env("CACHEON_NATIVE_ARTIFACT_ROOT")
        )
        return
    raise CuteCubinError(
