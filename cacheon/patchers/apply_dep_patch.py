...
``build`` writes only beneath ``CACHEON_NATIVE_ARTIFACT_STAGE``.  ``load`` performs
a side-effect-free validation of the exact publication mounted at
``CACHEON_NATIVE_ARTIFACT_ROOT``.  The complete native-build digest is the namespace;
...
def _log(message: str) -> None:
    print(f"[cacheon.apply_dep_patch] {message}", flush=True)
...
def _apply_to_overlay(policy, parsed_by_patch, site_root: Path, destination: Path) -> dict[str, str]:
    ...
    from cacheon.deppatch import apply_file_patch
...
def _materialize(
    ...
    *,
    ...
    from cacheon.dep_policy import dependency_site_root
...
def _required_target_architecture(policy) -> str:
    ...
    if not architectures:
        return ""
    actual = os.environ.get("CACHEON_TARGET_GPU_ARCH", "").strip().lower()
    if not actual and os.environ.get("CACHEON_REBUILD_PHASE", "all") == "all":
        import torch

        if not torch.cuda.is_available():
            raise RuntimeError(
                "dependency development rebuild requires a live CUDA device to "
                "derive CACHEON_TARGET_GPU_ARCH"
            )
        major, minor = torch.cuda.get_device_capability(torch.cuda.current_device())
        actual = f"sm{major}{minor}"
        os.environ["CACHEON_TARGET_GPU_ARCH"] = actual
...
def _build_prebuilt_modules(
    ...
    *,
    ...
    from cacheon.dep_policy import prebuilt_module_relative_path
...
def _development_workspace(build_spec_digest: str) -> Path:
    raw = os.environ.get("CACHEON_DEP_OVERLAY_CACHE", "").strip()
    base = Path(raw) if raw else Path.home() / ".cache" / "cacheon" / "dep_overlay"
    return base / "jit_workspace" / "v3" / build_spec_digest[:2] / build_spec_digest


def main() -> None:
    bundle_raw = os.environ.get("CACHEON_BUNDLE_PATH", "").strip()
    if not bundle_raw:
        _log("no CACHEON_BUNDLE_PATH set; nothing to apply")
        return
