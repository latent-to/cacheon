...
def _log(message: str) -> None:
    print(f"[cacheon.build_cuda_ext] {message}", flush=True)
...
def _phase() -> str:
    phase = os.environ.get("CACHEON_REBUILD_PHASE", "all").strip().lower()
    if phase not in _PHASES:
        raise CUDAExtensionError(f"unsupported CACHEON_REBUILD_PHASE: {phase!r}")
    return phase
    ...
    if match is None or match.group("suffix") not in {"", "a"} or (
        ...
        if required:
            raise CUDAExtensionError(
                "native build requires canonical CACHEON_TARGET_GPU_ARCH such as sm103 or sm120"
            )
        return ""
    return value
...
def _bundle_root() -> Path | None:
    raw = os.environ.get("CACHEON_BUNDLE_PATH", "").strip()
    if not raw:
        _log("no CACHEON_BUNDLE_PATH set; nothing to do")
        return None
    requested = Path(raw)
    try:
...
def _production_identity(phase: str) -> tuple[str, str, str]:
    build_spec = os.environ.get("CACHEON_NATIVE_BUILD_SPEC_DIGEST", "").strip()
    tree_digest = os.environ.get("CACHEON_ENGINE_TREE_DIGEST", "").strip()
    architecture = os.environ.get("CACHEON_TARGET_GPU_ARCH", "").strip()
    if phase in {"build", "load"}:
        return (
            _require_digest(build_spec, field="CACHEON_NATIVE_BUILD_SPEC_DIGEST"),
            _require_digest(tree_digest, field="CACHEON_ENGINE_TREE_DIGEST"),
...
def _compiler_environment() -> dict[str, str]:
    path = os.environ.get("CACHEON_BUILD_PATH", os.environ.get("PATH", "")).strip()
    if not path:
    ...
    temporary = os.environ.get("CACHEON_BUILD_TMPDIR", "/tmp").strip()
    if not temporary or not Path(temporary).is_absolute():
        raise CUDAExtensionError("CACHEON_BUILD_TMPDIR must be an absolute container path")
...
def _pinned_roots(context_roots: tuple[Path, ...], *, production: bool) -> list[str]:
    raw = os.environ.get("CACHEON_PINNED_BUILD_ROOTS", "").strip()
    if production and not raw:
        raise CUDAExtensionError("build phase requires CACHEON_PINNED_BUILD_ROOTS")
    roots: list[Path] = []
...
def _compile(
    *,
    ...
    timeout = float(os.environ.get("CACHEON_NATIVE_COMPILE_TIMEOUT_S", "1200"))
    if not (0 < timeout <= 7200):
        raise CUDAExtensionError("CACHEON_NATIVE_COMPILE_TIMEOUT_S is outside policy")
    try:
        subprocess.run(
            command,
            check=True,
...
def _build_phase(
    ...
    raw_stage = os.environ.get("CACHEON_NATIVE_ARTIFACT_STAGE", "").strip()
    if not raw_stage or not Path(raw_stage).is_absolute():
        raise CUDAExtensionError("build phase requires absolute CACHEON_NATIVE_ARTIFACT_STAGE")
...
def _load_phase(
    ...
    if os.environ.get("CACHEON_ENGINE_WORKER") != "1":
        raise CUDAExtensionError("native load phase requires an isolated engine worker")
    raw_root = os.environ.get("CACHEON_NATIVE_ARTIFACT_ROOT", "").strip()
    publication_digest = _require_digest(
        os.environ.get("CACHEON_NATIVE_ARTIFACT_PUBLICATION_DIGEST", "").strip(),
        field="CACHEON_NATIVE_ARTIFACT_PUBLICATION_DIGEST",
    )
    if not raw_root or not Path(raw_root).is_absolute():
        raise CUDAExtensionError("load phase requires absolute CACHEON_NATIVE_ARTIFACT_ROOT")
...
def _development_all(
    ...
    raw_cache = os.environ.get("CACHEON_CUDA_EXT_CACHE", "").strip()
    cache_root = Path(raw_cache) if raw_cache else Path.home() / ".cache" / "cacheon" / "cuda_ext"
