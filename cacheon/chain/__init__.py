...
The exact SDK methods called here are pinned by ``cacheon chain-compat``
(cacheon/chain_canary.py); run it after any bittensor bump, and the wrappers' calls
...
def _decode_reveal_candidates(candidates: tuple[bytes, ...]) -> str:
    ...
    for encoded in candidates:
        ...
        if mode == 0:
            ...
            # Modes 2/3 encode at least 16 KiB, beyond Cacheon's 4 KiB bound.
            continue
        if (
            (mode == 1 and size < 64)
...
def preflight(subtensor, wallet, netuid: int) -> list:
    ...
    from cacheon.chain_canary import Check
