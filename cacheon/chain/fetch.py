...
accepts only members covered by :func:`cacheon.bundle_hash.content_hash`; ignored
...
logger = logging.getLogger("cacheon.chain.fetch")
...
def _resolve_addresses(hostname: str, port: int, *, deadline: float) -> tuple[str, ...]:
    ...
    threading.Thread(target=resolve, name="cacheon-bundle-dns", daemon=True).start()
    try:
        ...
        try:
            ...
            connection.request(
                ...
                headers={
                    ...
                    "Connection": "close",
                    "User-Agent": "cacheon-validator-bundle-fetch/1",
                },
            )
            wire_socket.settimeout(_remaining(deadline))
