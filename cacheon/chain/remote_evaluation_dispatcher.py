...
class AuthenticatedRemoteEvaluationResponse:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            != ("arena_screen_receipt" if self.stage == "screen" else "qualification_intake_batch")
            or type(self.schema_version) is not int
            or self.schema_version != _SCHEMA_VERSION
            or not isinstance(self.auth_tag, str)
            or _AUTH_TAG.fullmatch(self.auth_tag) is None
...
def _payload_encoding(payload: object) -> tuple[str, bytes, str]:
    ...
    if type(payload) is QualificationIntakeBatch:
        return (
            "qualification_intake_batch",
            canonical_json_bytes(qualification_batch_to_dict(payload)),
            qualification_batch_payload_digest(payload),
        )
...
def _validate_request_body(stage: str, value: dict[str, object]) -> None:
    if stage == "screen":
        ...
        if set(value) != fields or value["kind"] != "screen_work" or value["schema_version"] != 1:
    ...
    try:
        manifest = QualificationAuthorityManifest.from_dict(value["authority_manifest"])
    ...
    if tuple(reservations) != manifest.reservations:
        raise RemoteEvaluationDispatcherError(
            "qualification request differs from its authority manifest"
        )
...
def _request_body_for_qualification(
    ...
    return {
        ...
        "qualification_policy_digest": work.qualification_policy_digest,
        "schema_version": _SCHEMA_VERSION,
        "service_digest": coordinator.service.identity,
    }
...
def reopen_remote_response(
    ...
    transport_identity: RemoteWorkerTransportIdentity,
    credential: RemoteWorkerCredential,
) -> ArenaScreenReceipt | QualificationIntakeBatch:
    ...
    if response.stage == "screen":
        ...
        payload = qualification_batch_from_dict(response.payload)
        observed_digest = qualification_batch_payload_digest(payload)
...
class RemoteEvaluationDispatcher:
    ...
    def dispatch_qualification_once(self) -> EvaluationRun | None:
        ...
        prepared = None
        try:
            work = self.coordinator.service.plan_qualification(
                claim.candidates, claim.screen_receipts, state=None
            )
            self.coordinator._validate_work(work, claim)
            try:
            ...
            request = seal_remote_request(
                ...
                self.transport_identity,
                self.credential,
                _request_body_for_qualification(self.coordinator, claim, work),
            )
            response = self.transport.run_qualification(
                request,
                job=claim,
                work=work,
                prepared=prepared,
            )
            batch = reopen_remote_response(
                request, response, self.transport_identity, self.credential
            )
            if type(batch) is not QualificationIntakeBatch:
                raise RemoteEvaluationDispatcherError(
            ...
            self.coordinator._validate_batch(batch, work, claim)
            if prepared is None and not self.coordinator._systemic(batch):
                prepared = work.factory.build()
        except BaseException as exc:
            lease, heartbeat_error = heartbeat.stop()
            self._release_after_remote_error(
        ...
        if prepared is None:
            self._release_after_remote_error(
                lease,
                reason="remote_qualification_stack_authority_missing",
                cause=EvaluationCoordinatorError(
                    "qualification completed without rebuildable stack authority"
                ),
                result_digest=envelope.digest,
            )
        self.coordinator.commit_qualification_result(
            claim, work, prepared, batch, envelope
        )
