...
``cacheon/eval/engine_worker.py`` (in-worker session + isolation probes) and
``cacheon/eval/oci_session_worker.py`` (the no-egress worker).
"""
...
def call_in_subprocess(fn, *args, **kwargs):
    ...
    fd, path = tempfile.mkstemp(prefix="cacheon_launch_", suffix=".pkl")
    os.close(fd)
    try:
