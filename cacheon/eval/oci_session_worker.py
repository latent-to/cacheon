...
_DIGEST_ENV = {
    "runtime_digest": "CACHEON_RUNTIME_DIGEST",
    "worker_distribution_digest": "CACHEON_WORKER_DISTRIBUTION_DIGEST",
    "model_revision_digest": "CACHEON_MODEL_REVISION_DIGEST",
    "model_manifest_digest": "CACHEON_MODEL_MANIFEST_DIGEST",
    "model_content_digest": "CACHEON_MODEL_CONTENT_DIGEST",
}
...
def _write_stderr_diagnostic(stage: str, exc: BaseException) -> None:
    ...
    try:
        ...
        raw = (
            f"[cacheon-session-worker] stage={stage[:64]!r} "
...
def _required_sglang_version() -> str:
    value = os.environ.get("CACHEON_EXPECTED_SGLANG_VERSION", "").strip()
    if (
        not value
        or len(value) > 256
...
def _artifact_root() -> Path:
    raw = os.environ.get("CACHEON_NATIVE_ARTIFACT_ROOT", "").strip()
    if not raw:
        raise SessionWorkerError("CACHEON_NATIVE_ARTIFACT_ROOT is missing")
    requested = Path(raw)
...
def _validate_live_preflight(
    ...
    if os.environ.get("CACHEON_LAUNCH_DIGEST", "").strip() != launch_digest:
        raise SessionWorkerError("live launch digest differs from the init binding")
    if _required_digest_env("CACHEON_ENGINE_CONFIG_DIGEST") != config.digest:
        raise SessionWorkerError("live engine configuration differs from the init binding")
    expected_tree = _required_digest_env("CACHEON_ENGINE_TREE_DIGEST")
    expected_stack = _required_digest_env("CACHEON_STACK_DIGEST")
...
def _prepare_descendant_bootstrap() -> None:
    ...
    import cacheon.bootstrap  # noqa: F401 - deliberately after live preflight
...
def run_session(*, input_fd: int = 0, output_fd: int | None = None) -> int:
    ...
    try:
        ...
        session_id, launch_digest, config = validate_init(
            init,
            expected_launch_digest=os.environ.get("CACHEON_LAUNCH_DIGEST", ""),
            expected_engine_config_digest=os.environ.get(
                "CACHEON_ENGINE_CONFIG_DIGEST", ""
            ),
        )
