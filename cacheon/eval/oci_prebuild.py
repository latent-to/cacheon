...
from cacheon.eval.oci_process import OCILease, OCIProcessManager
...
def build_prebuild_argv(
    *,
    ...
    env = {
        ...
        "CACHEON_ENGINE_TREE_DIGEST": resolved.spec.tree_digest,
        "CACHEON_BUILD_PATH": ":".join(policy.build_path),
        "CACHEON_BUILD_TMPDIR": policy.build_tmpdir,
        "CACHEON_NATIVE_ARTIFACT_STAGE": CONTAINER_STAGE,
        "CACHEON_NATIVE_BUILD_SPEC_DIGEST": resolved.native_build_spec.digest,
        "CACHEON_NATIVE_COMPILE_TIMEOUT_S": str(policy.native_compile_timeout_seconds),
        "CACHEON_PINNED_BUILD_ROOTS": ":".join(policy.pinned_build_roots),
...
def run_oci_prebuild(
    ...
    *,
    ...
    lease = manager.register(
        lease_id=lease_id,
        container_name="cacheon-" + lease_id,
    ...
    try:
        ...
        reopen_launch_tree(launch, resolved.materialized_tree_root)
        stage_receipt = _read_prebuild_receipt(stage_path, resolved=resolved)
        ...
        receipt = _read_prebuild_receipt(publication.root, resolved=resolved)
        if receipt != stage_receipt:
            raise OCIPrebuildError(
...
def container_build() -> Path:
    ...
    build_digest = _container_value("CACHEON_NATIVE_BUILD_SPEC_DIGEST", pattern=re.compile(r"[0-9a-f]{64}"))
    tree_digest = _container_value("CACHEON_ENGINE_TREE_DIGEST", pattern=re.compile(r"[0-9a-f]{64}"))
    architecture = _container_value("CACHEON_TARGET_GPU_ARCH", pattern=_ARCH)
    stage_raw = _container_value("CACHEON_NATIVE_ARTIFACT_STAGE")
    roots_raw = _container_value("CACHEON_PINNED_BUILD_ROOTS")
    build_path_raw = _container_value("CACHEON_BUILD_PATH")
    build_tmpdir = _container_value("CACHEON_BUILD_TMPDIR")
    ...
    preserved = {
        ...
        "HOME": "/tmp/home",
        "NVIDIA_VISIBLE_DEVICES": "void",
        "CACHEON_BUILD_PATH": build_path_raw,
        "CACHEON_BUILD_TMPDIR": build_tmpdir,
        "CACHEON_BUNDLE_PATH": CONTAINER_TREE,
        "CACHEON_ENGINE_TREE_DIGEST": tree_digest,
        "CACHEON_NATIVE_ARTIFACT_STAGE": CONTAINER_STAGE,
        "CACHEON_NATIVE_BUILD_SPEC_DIGEST": build_digest,
        "CACHEON_NATIVE_COMPILE_TIMEOUT_S": compile_timeout,
        "CACHEON_PINNED_BUILD_ROOTS": roots_raw,
        "CACHEON_REBUILD_CONTAINER": "1",
        "CACHEON_REBUILD_PHASE": "build",
        "CACHEON_TARGET_GPU_ARCH": architecture,
        "PATH": build_path_raw,
        "PYTHONDONTWRITEBYTECODE": "1",
        "PYTHONHASHSEED": "0",
        "TMPDIR": "/tmp",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="python -m cacheon.eval.oci_prebuild")
