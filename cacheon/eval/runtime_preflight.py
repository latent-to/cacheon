...
The probe inspects installed distribution metadata without importing Cacheon,
Torch, SGLang, or candidate code.  It hashes the installed Cacheon distribution
...
_CONTAINER_NAME = re.compile(r"cacheon-runtime-preflight-[0-9a-f]{20}\Z")
_PACKAGE_NAMES = (
    "cuda-python",
    "flashinfer-python",
...
if len(worker_dists) != 1:
    raise RuntimeError("expected exactly one installed Cacheon worker distribution")
dist = worker_dists[0]
files = dist.files
...
for entry in files:
    ...
    # ../../../bin/cacheon.  They are intentionally outside the in-image worker
    ...
    has_module = has_module or selected == "cacheon/__init__.py"
...
def _new_container_name() -> str:
    try:
        name = "cacheon-runtime-preflight-" + secrets.token_hex(10)
