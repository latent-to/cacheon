...
logger = logging.getLogger("cacheon.flashinfer_overlay")
...
class _LoadOnlyJITSpec:
    ...
    def build_and_load(self):
        from cacheon.dep_policy import sha256_file
...
def _active_overlays():
    ...
    from cacheon.dep_policy import PATCHABLE_DEPS, read_validated_overlay
...
def install(registry) -> None:  # registry unused; signature shared by integrations
    ...
    for overlay, policy in overlays:
        if policy.env_rebind is not None:
            ...
            logger.info("cacheon: %s.%s -> %s", module_name, attribute, overlay.subtree)
        target_architecture = os.environ.get("CACHEON_TARGET_GPU_ARCH", "").strip().lower()
    ...
    print(
        f"[cacheon] dep overlay ACTIVE: targets={applied} "
