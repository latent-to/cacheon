"""Wire the Cacheon dispatcher into SGLang's attention layer.
...
See ``cacheon/dispatch.make_attention_dispatcher`` for the scope of what's wired
today (the self-contained case, gated behind ``CACHEON_ATTENTION_SEAM=1``) versus
the paged-decode / MLA-latent GPU integration point.
"""

from __future__ import annotations

from cacheon.dispatch import make_attention_dispatcher
from cacheon.registry import REGISTRY, KernelRegistry

_PATCH_FLAG = "_cacheon_attn_patched"
...
def install(registry: KernelRegistry = REGISTRY) -> None:
    ...
    RadixAttention._cacheon_orig_forward = orig_forward  # type: ignore[attr-defined]
...
def uninstall() -> None:
    ...
    if not getattr(RadixAttention, _PATCH_FLAG, False):
        return
    RadixAttention.forward = RadixAttention._cacheon_orig_forward  # type: ignore[attr-defined]
    delattr(RadixAttention, "_cacheon_orig_forward")
