"""Wire the Cacheon dispatcher into the MSA PREFILL indexer waist.
...
_PATCH_FLAG = "_cacheon_msa_prefill_patched"
_SOURCE_MODULE = (
    "sglang.srt.layers.attention.minimax_sparse_ops.prefill.flash_with_topk_idx"
)
_CONSUMER_MODULE = "sglang.srt.layers.attention.minimax_sparse_ops.minimax_sparse"
_FUNC = "flash_prefill_with_topk_index"
_ORIG_ATTR = "_cacheon_orig_flash_prefill_with_topk_index"
_DISPATCH_ATTR = "_cacheon_dispatch_flash_prefill_with_topk_index"
...
def install(registry: KernelRegistry = REGISTRY) -> None:
    ...
    if hasattr(source, _ORIG_ATTR) or hasattr(source, _DISPATCH_ATTR):
        raise RuntimeError("MSA prefill defining module has stale Cacheon patch state")
