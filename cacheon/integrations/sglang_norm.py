"""Bind registered RMSNorm contracts before SGLang resolves the layer methods."""

from __future__ import annotations

from cacheon.dispatch import make_rmsnorm_dispatcher
from cacheon.registry import REGISTRY, KernelRegistry

_PATCH_FLAG = "_cacheon_patched"


def install(registry: KernelRegistry = REGISTRY) -> None:
    """Patch RMSNorm.forward_cuda/native. No-ops until layernorm is imported."""
    import sys

    mod = sys.modules.get("sglang.srt.layers.layernorm")
    RMSNorm = getattr(mod, "RMSNorm", None) if mod is not None else None
    if RMSNorm is None:
        return

    if getattr(RMSNorm, _PATCH_FLAG, False):
        return

    orig_cuda = RMSNorm.forward_cuda
    orig_native = RMSNorm.forward_native

    RMSNorm.forward_cuda = make_rmsnorm_dispatcher(orig_cuda, registry=registry)
    RMSNorm.forward_native = make_rmsnorm_dispatcher(orig_native, registry=registry)

    RMSNorm._cacheon_orig_cuda = orig_cuda  # type: ignore[attr-defined]
    RMSNorm._cacheon_orig_native = orig_native  # type: ignore[attr-defined]
    setattr(RMSNorm, _PATCH_FLAG, True)


def uninstall() -> None:
    import sys

    if "sglang.srt.layers.layernorm" not in sys.modules:
        return
    from sglang.srt.layers.layernorm import RMSNorm

    if not getattr(RMSNorm, _PATCH_FLAG, False):
        return
    RMSNorm.forward_cuda = RMSNorm._cacheon_orig_cuda  # type: ignore[attr-defined]
    RMSNorm.forward_native = RMSNorm._cacheon_orig_native  # type: ignore[attr-defined]
    delattr(RMSNorm, "_cacheon_orig_cuda")
    delattr(RMSNorm, "_cacheon_orig_native")
    setattr(RMSNorm, _PATCH_FLAG, False)
