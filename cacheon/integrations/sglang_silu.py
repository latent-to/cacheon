...
def install(registry: KernelRegistry = REGISTRY) -> None:
    ...
    SiluAndMul._cacheon_orig_cuda = orig_cuda  # type: ignore[attr-defined]
    SiluAndMul._cacheon_orig_native = orig_native  # type: ignore[attr-defined]
...
def uninstall() -> None:
    ...
    if not getattr(SiluAndMul, _PATCH_FLAG, False):
        return
    SiluAndMul.forward_cuda = SiluAndMul._cacheon_orig_cuda  # type: ignore[attr-defined]
    SiluAndMul.forward_native = SiluAndMul._cacheon_orig_native  # type: ignore[attr-defined]
    delattr(SiluAndMul, "_cacheon_orig_cuda")
    delattr(SiluAndMul, "_cacheon_orig_native")
