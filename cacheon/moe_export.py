...
validator-owned seam integrations share:
  ...
  * the arfusion dispatcher (cacheon/dispatch.py) — CONSUMES: a call whose input
...
Layouts match the slot contract (cacheon/slots.py): gemm ``[T*K, H]`` per-rank partial,
...
logger = logging.getLogger("cacheon.moe_export")
...
# CACHEON_DEEP_DEBUG=1: trace every arm/refusal/export/consume with the stream-
...
_DEBUG = os.environ.get("CACHEON_DEEP_DEBUG", "") == "1"
...
def on_forward_boundary(forward_batch: Any) -> None:
    ...
    if _state["pends"]:
        ...
        logger.warning("cacheon: deep seam dropped %d abandoned export pend(s) at "
...
def flashinfer_tuning() -> bool:
    ...
    Every Cacheon seam inside that stock pipeline must stay invisible while this is
...
def _disable(reason: str) -> None:
    if not _state["disabled"]:
        ...
        logger.warning("cacheon: deep export seam DISABLED: %s", reason)
...
def _num_layers() -> Optional[int]:
    ...
    try:
        ...
        if pp not in (None, 0, 1):
            logger.warning("cacheon: deep seam needs a per-rank layer count; "
        ...
        if spec_name not in ("NONE", ""):
            logger.warning("cacheon: deep seam disabled under speculative decoding "
        ...
        for holder in holders:
            ...
            if isinstance(n, int) and n > 0:
                logger.warning("cacheon: deep seam layer count = %d (last-layer "
                               "veto active)", n)
                _state["n_layers"] = n
                return n
        logger.warning("cacheon: deep seam found no num_hidden_layers in %s "
                       "(top-level keys: %s)", srv.model_path,
                       sorted(raw.keys())[:12])
    except Exception as exc:  # noqa: BLE001
        logger.warning("cacheon: deep seam could not resolve num_hidden_layers: %s",
                       exc)
    return None
...
def maybe_export(orig: Callable[..., Any], args: tuple, kwargs: dict, *,
    ...
    from cacheon.dispatch import _arch_tag, _dtype_name, _in_cuda_graph
    ...
    if not (
        ...
        raise RuntimeError(
            "cacheon deep seam: export ABI mismatch "
    ...
    if optr in _state["pends"]:
        raise RuntimeError(
            f"cacheon deep seam: moe output buffer {optr:#x} reused before its export "
    ...
    if len(_state["pends"]) > _MAX_PENDS:
        raise RuntimeError("cacheon deep seam: export pends leaking (never consumed)")
    _state["pends"][optr] = {
        "g2": g2,
        "idx": rmap,
        "scl": scl,
...
def consume(input_tensor: torch.Tensor) -> Optional[dict]:
    ...
    if exp is not None:
        ...
        if _state["consumed"] == 1:
            logger.warning("cacheon: deep epilogue consume path LIVE (T_export=%d K=%d)",
                           exp["T"], exp["K"])
            _receipt()
    return exp
...
def orphaned(exp: dict) -> None:
    ...
    _state["orphans"] += 1
    logger.warning("cacheon: deep export ORPHANED (T=%d K=%d) — trusted reconstruct "
...
def _wrap_ptr(ptr: int, shape: tuple, typestr: str, device: torch.device,
    ...
    if t.data_ptr() != int(ptr):  # a copy would silently detach us from the workspace
        raise RuntimeError("cacheon deep seam: pointer wrap copied instead of aliasing")
...
def trusted_finalize(exp: dict, input_tensor: torch.Tensor) -> torch.Tensor:
    ...
    from cacheon.slots import _moe_fin_local_finalize
