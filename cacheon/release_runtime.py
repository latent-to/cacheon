"""Fail-closed entrypoint for a signed Cacheon Engine serving release.
...
_RECEIPT_ROOT = Path("/tmp/cacheon-release-receipts")
    ...
    *, expected_sglang_version: str, expected_upstream_revision: str
    ...
    for row in rows:
        ...
        if (
            ...
            or not source_name.startswith("cacheon/arena_assets/minimax_m3/sglang_patch/")
...
def install_reviewed_runtime_overlays(
    *, expected_sglang_version: str, expected_upstream_revision: str
    ...
    for _source_name, target_name, source_payload, target in inventory:
        ...
        temporary = destination.with_name(f".{destination.name}.cacheon-overlay")
        try:
            fd = os.open(
                temporary,
...
def verify_serving_release(
    *,
    ...
    from cacheon.model_provision import reopen_embedded_model_provision
    from cacheon.release import ReleaseError, reopen_release
    ...
    environment = {
        **dict(descriptor.serve.environment),
        "CACHEON_ACTIVE": "1",
        "CACHEON_BUNDLE_PATH": "/cacheon/engine-tree",
        "CACHEON_ENGINE_TREE_DIGEST": descriptor.engine_tree_digest,
        "CACHEON_ENGINE_WORKER": "1",
        "CACHEON_NATIVE_ARTIFACT_PUBLICATION_DIGEST": descriptor.native.publication_digest,
        "CACHEON_NATIVE_ARTIFACT_ROOT": str(native_root),
        "CACHEON_NATIVE_BUILD_SPEC_DIGEST": descriptor.native.build_spec_digest,
        "CACHEON_PREBUILT_ARTIFACTS": "1",
        "CACHEON_REBUILD_PHASE": "load",
        "CACHEON_RELEASE_DESCRIPTOR_DIGEST": descriptor.digest,
        "CACHEON_RELEASE_REQUIRED": "1",
        "CACHEON_RELEASE_VERIFIED": descriptor.digest,
        "CACHEON_STACK_DIGEST": descriptor.release_manifest.digest,
        "CACHEON_STRICT": "1",
        "CACHEON_TARGET_GPU_ARCH": descriptor.native.build_spec["target_architecture"],
        "CACHEON_MODEL_CONTENT_DIGEST": descriptor.model.content_digest,
        "CACHEON_MODEL_MANIFEST_DIGEST": descriptor.model.manifest_digest,
        "CACHEON_RUNTIME_DIGEST": descriptor.release_manifest.runtime_digest,
        "CACHEON_WORKER_DISTRIBUTION_DIGEST": descriptor.native.build_spec["worker_distribution_digest"],
        "CACHEON_EXPECTED_SGLANG_VERSION": descriptor.sglang_version,
        "CACHEON_SEAM_RECEIPT_DIR": str(_RECEIPT_ROOT),
        "SGLANG_PLUGINS": "cacheon",
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="verify and enter a signed Cacheon Engine release")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if command and command[0] == "--":
        command = command[1:]
    if Path(args.release_root).resolve() != Path("/cacheon"):
        raise ReleaseRuntimeError("production release root must be /cacheon")
...
if __name__ == "__main__":
    try:
        ...
        print(f"cacheon release refused: {exc}", file=sys.stderr)
