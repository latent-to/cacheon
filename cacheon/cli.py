...
def cmd_chain_compat(_: argparse.Namespace) -> int:
    from cacheon.chain_canary import format_checks, run_checks
...
def _object_store_from_args(args: argparse.Namespace):
    ...
    if provider in {"hippius", "s3", "minio"} and not bucket:
        raise SystemExit(
            f"--object-store-bucket or CACHEON_OBJECT_STORE_BUCKET is required "
            f"for provider={provider}"
        )
    if provider == "local" and not root:
        raise SystemExit(
            "--object-store-root or CACHEON_OBJECT_STORE_ROOT_DIR is required "
            "for provider=local"
        )
    ...
    cfg = ObjectStoreConfig(
        ...
        region_name=_opt("object_store_region", "CACHEON_OBJECT_STORE_REGION"),
        access_key_id=_opt(
            "object_store_access_key", "CACHEON_OBJECT_STORE_ACCESS_KEY_ID"
        ),
        secret_access_key=_opt(
            "object_store_secret_key", "CACHEON_OBJECT_STORE_SECRET_ACCESS_KEY"
        ),
        addressing_style=_opt(
            "object_store_addressing", "CACHEON_OBJECT_STORE_ADDRESSING_STYLE"
        ),
...
def _s3_store_config_from_args(
    ...
    *,
    ...
    if not bucket:
        raise SystemExit(
            "--object-store-bucket or CACHEON_OBJECT_STORE_BUCKET is required"
        )
...
def _validator_archive_store_from_args(args: argparse.Namespace):
    ...
    from cacheon.object_store import open_configured_object_store

    config = _s3_store_config_from_args(
        args,
        default_prefix=DEFAULT_VALIDATOR_ARCHIVE_PREFIX,
        prefix_env="CACHEON_VALIDATOR_ARCHIVE_PREFIX",
    )
...
def _add_object_store_args(parser: argparse.ArgumentParser) -> None:
    parser.add_argument(
        ...
        help=(
            ...
            "(default: none; env CACHEON_OBJECT_STORE_PROVIDER). "
    ...
    parser.add_argument(
        ...
        default="",
        help="secret key (or CACHEON_OBJECT_STORE_SECRET_ACCESS_KEY / AWS_SECRET_ACCESS_KEY)",
    )
    parser.add_argument(
...
def cmd_mint_weight_gateway(args: argparse.Namespace) -> int:
    ...
    secrets_payload = {
        "schema": "cacheon.weight-gateway-secrets.v1",
    ...
    manifest = {
        "schema": "cacheon.weight-gateway-authority.v1",
        ...
        "serve_example": (
            f"cacheon serve-weights --wallet {wallet_name} --hotkey {hotkey_name} "
        ...
        "follow_pin_example": (
            f"cacheon follow-weights --url http://<gateway-host>:8080 "
...
def cmd_push_weight_offer(args: argparse.Namespace) -> int:
    """Eval: build the current legacy V1 weight offer and push it; never publish on-chain."""

    import os

    from cacheon import chain
    from cacheon.chain.intake import FinalizedIntakeStore, IntakeError, IntakeScope
    from cacheon.chain.weight_push_auth import (
...
def cmd_chain_submit(args: argparse.Namespace) -> int:
    from cacheon.chain.eval_cost import EvalCostError, EvalCostFetchError, EvalCostPolicy
    ...
    if res.get("paid"):
        ...
        if payment.get("dry_run"):
            print(
                "eval_cost payment is quoted only; --dry-run does not transfer TAO."
            )
...
def cmd_chain_validate(
    ...
    # "cacheon.chain.*" loggers (--once prints its own summary below). This must run
    ...
    # cacheon.chain.validator read level=50). Own the subtree outright: reset levels
    # to inherit, dedicated handler, no propagation upward.
    for _name, _lg in list(logging.root.manager.loggerDict.items()):
        if _name.startswith("cacheon.") and isinstance(_lg, logging.Logger):
            _lg.disabled = False
            _lg.setLevel(logging.NOTSET)
    _chain_lg = logging.getLogger("cacheon.chain")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    sp.add_argument(
        ...
        help=(
            ...
            "from the eval host (or CACHEON_WEIGHT_PUSH_CREDENTIALS / "
            "CACHEON_WEIGHT_PUSH_KEY); omit to disable push"
        ),
    )
    ...
    sp.add_argument(
        "--wallet-path",
        required=True,
        help="Bittensor wallets root to create (e.g. /var/lib/cacheon/wallets)",
    )
    ...
    sp.add_argument(
        ...
        help=(
            ...
            "(or CACHEON_WEIGHT_PUSH_CREDENTIALS / CACHEON_WEIGHT_PUSH_KEY)"
        ),
    )
    sp.add_argument(
        "--credential-id",
    ...
    sp.add_argument(
        "--object-store-bucket",
        default="",
        help="miner-owned bucket (or CACHEON_OBJECT_STORE_BUCKET)",
    )
    sp.add_argument(
        "--object-store-prefix",
        default="",
    ...
    sp.add_argument(
        ...
        type=int,
        default=1_000_000_000,
        help="TAO amount in rao to transfer with --pay; default 1000000000 (1 TAO)",
    )
