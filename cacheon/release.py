...
from cacheon.engine_tree import MaterializedEngineTree, reopen_materialized_engine_tree
...
RUNTIME_DISTRIBUTION = "cacheon-engine"
RUNTIME_VERSION = "0.0.1"
RUNTIME_WHEEL = "cacheon_engine-0.0.1-py3-none-any.whl"
SOURCE_ARCHIVE = "cacheon-engine-source-0.0.1.tar.gz"
...
_FORBIDDEN_ENV_PREFIXES = (
    "CACHEON_", "PYTHON", "LD_", "BT_", "BITTENSOR", "WALLET", "MINER",
)
...
_RUNTIME_EXACT = frozenset(
    {
        "cacheon/eval/__init__.py",
        "cacheon/eval/native_artifact.py",
        "cacheon/eval/engine_launch.py",
        "cacheon/eval/evidence_store.py",
        "cacheon/eval/calibration.py",
        "cacheon/eval/qualification.py",
...
class ModelReleaseIdentity:
    ...
    def from_provisioning(
        ...
        from cacheon.model_provision import ModelProvisionReceipt
...
class NativeReleaseIdentity:
    ...
    def __post_init__(self) -> None:
        from cacheon.eval.engine_launch import NativeBuildSpec
...
def _runtime_files(source_root: str | Path) -> dict[str, bytes]:
    ...
    candidates = [*(root / "cacheon").rglob("*"), *(root / "cacheon_kernels").rglob("*"), *(root / "LICENSES").rglob("*")]
    ...
    for path in sorted(set(candidates)):
        ...
        include = (
            ...
            or (
                relative.startswith("cacheon/")
                and PurePosixPath(relative).parent == PurePosixPath("cacheon")
    ...
    required = {
        "cacheon/__init__.py",
        "cacheon/bootstrap.py",
        "cacheon/seam.py",
        "cacheon/integrations/sglang_plugin.py",
        "cacheon_kernels/__init__.py",
        "LICENSE",
        "NOTICE",
    }
    if not required <= set(files):
        raise ReleaseError(f"runtime release source is incomplete: {sorted(required - set(files))}")
    banned = ("cacheon/chain/", "cacheon/commit_reveal.py", "cacheon/economics.py", "cacheon/settlement.py", "cacheon/cli.py")
...
def _tar_gz(files: Mapping[str, bytes]) -> bytes:
    ...
    with tarfile.open(fileobj=raw, mode="w", format=tarfile.PAX_FORMAT) as archive:
        for path, payload in sorted(files.items()):
            info = tarfile.TarInfo(f"cacheon-engine-{RUNTIME_VERSION}/{path}")
...
def _wheel(files: Mapping[str, bytes]) -> bytes:
    dist = f"cacheon_engine-{RUNTIME_VERSION}.dist-info"
    payloads = dict(files)
    payloads["cacheon_engine_bootstrap.pth"] = b"import cacheon.bootstrap\n"
...
def prepare_release(
    *,
    ...
    from cacheon.eval.engine_launch import NativeBuildSpec
    from cacheon.eval.native_artifact import NativeArtifactPublication, reopen_native_artifact
    from cacheon.model_provision import ProvisionedModel
...
def _reopen_calibration_manifest(payload: bytes) -> object:
    from cacheon.eval.calibration import CalibrationManifest
...
def _container_deployment(
    ...
    return {
        ...
        "serve_receipt_directory": "/tmp/cacheon-release-receipts",
...
def _container_dockerfile(release: PublishedRelease, overlay_digest: str) -> bytes:
    ...
    return (
        ...
        "COPY release/artifacts/" + RUNTIME_WHEEL + " /tmp/cacheon-engine.whl\n"
        "RUN /usr/bin/python3 -m pip install --no-deps /tmp/cacheon-engine.whl && rm /tmp/cacheon-engine.whl\n"
        + "RUN /usr/bin/python3 -m cacheon.release_runtime install-reviewed-overlays"
        ...
        "COPY release /cacheon\n"
        "COPY trusted-release-key /etc/cacheon/trusted-release-key\n"
        "COPY seccomp.json /etc/cacheon/seccomp.json\n"
        + "".join(
...
def container_context(
    ...
    from cacheon.release_runtime import reviewed_runtime_overlay_digest
