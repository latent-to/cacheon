...
Layers, and what covers what:
  ...
  * Declared ``cuda_sources`` (the sanctioned CUDA-source tier, see
    ``cacheon/manifest.py``) get only TWO of the three: an exact sha256 of the raw
    ...
    ``cacheon/rebuild.py`` before compilation, which is a stronger check than an advisory
...
def bundle_slot_file_fingerprints(bundle_root: str | Path) -> dict[str, list[str]]:
    ...
    Also folds in the op's declared ``cuda_sources`` (see ``cacheon/manifest.py``): each
