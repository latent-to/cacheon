    ...
    bundle/
      ...
      kernels/
        ...
        silu_and_mul.cu        # optional: declared via ops.cuda_sources (sanctioned
                                ...
                                # validator-reviewed patcher, see cacheon/rebuild.py)
      metadata/
...
class AOTExport:
    ...
    :class:`~cacheon.artifact_abi.SlotCallABI` plus declared validator-allocated
...
class CompetitionEntry:
    ...
    are resolved independently by :mod:`cacheon.target_catalog`.
    """
...
def load_manifest(bundle_root: str | Path) -> Manifest:
    ...
    for i, op in enumerate(ops_raw):
        ...
        # are resolved against cacheon_kernels at load (here we only check structure).
...
def all_declared_cuda_sources(bundle_root: str | Path, manifest: Manifest) -> frozenset[Path]:
    ...
    The shape ``cacheon.sandbox.scan_tree`` wants for its declared-allowlist parameter:
