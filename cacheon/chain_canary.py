...
Cacheon's chain layer (commitments read, weight setting, metagraph, registration
preflight) calls into the bittensor SDK. The SDK's surface drifts across releases,
so — exactly like the sglang seam canary (``cacheon compat``) — we introspect the
...
Run ``cacheon chain-compat``. Green means the SDK has what we expect; red lists what
...
# The subtensor methods Cacheon's chain layer intends to call. We check the class
