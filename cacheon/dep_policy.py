...
def rebuild_phase() -> str:
    ...
    if phase not in {"all", "build", "load"}:
        raise DepOverlayError(f"unsupported CACHEON_REBUILD_PHASE: {phase!r}")
    return phase
...
def native_build_spec_digest(*, phase: str | None = None) -> str:
    ...
    raw = os.environ.get("CACHEON_NATIVE_BUILD_SPEC_DIGEST", "").strip()
    if not raw:
        raise DepOverlayError(
            f"CACHEON_NATIVE_BUILD_SPEC_DIGEST is required in rebuild phase {phase!r}"
        )
    return _digest(raw, field="CACHEON_NATIVE_BUILD_SPEC_DIGEST")
...
def resolved_build_spec_digest(bundle: str | Path, *, phase: str | None = None) -> str:
    ...
    phase = phase or rebuild_phase()
    if os.environ.get("CACHEON_NATIVE_BUILD_SPEC_DIGEST", "").strip():
...
def native_artifact_root(*, phase: str | None = None, build_spec_digest: str | None = None) -> Path:
    ...
    phase = phase or rebuild_phase()
    if phase == "build":
        return _absolute_directory_env("CACHEON_NATIVE_ARTIFACT_STAGE")
    if phase == "load":
        return _absolute_directory_env("CACHEON_NATIVE_ARTIFACT_ROOT")

    digest = build_spec_digest or native_build_spec_digest(phase=phase)
    base_raw = os.environ.get("CACHEON_DEP_OVERLAY_CACHE", "").strip()
    base = Path(base_raw) if base_raw else Path.home() / ".cache" / "cacheon" / "dep_overlay"
    if not base.is_absolute():
        raise DepOverlayError("CACHEON_DEP_OVERLAY_CACHE must be absolute")
