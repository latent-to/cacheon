...
from optima.chain.publication import reopen_worker_bundle
from optima.eval.evidence_store import EvidenceArtifactRef, reopen_evidence
...
class ArchivedTree:
    ...
    def digest(self) -> str:
        return canonical_digest(
            "optima.validator.archived-tree",
...
class ValidatorArchiveManifest:
    ...
    def digest(self) -> str:
        return canonical_digest(
            "optima.validator.archive-manifest",
            self.to_dict(),
        )
...
def create_validator_archive(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="optima-validator-archive.") as temporary:
    ...
    try:
        ...
        for item in manifest.qualification_evidence:
            root_id = canonical_digest(
                "optima.validator.archived-evidence-root",
            ...
            canonical_json_bytes(
                {
                    ...
                    "mappings": mappings,
                    "schema": "optima.validator-archive-restore-map.v1",
                }
            ),
            0o600,
        )
...
def verify_validator_archive(
    ...
    with tempfile.TemporaryDirectory(prefix="optima-validator-restore.") as temporary:
