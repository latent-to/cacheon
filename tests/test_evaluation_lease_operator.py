...
def _arrival(index: int, *, block: int = BLOCK) -> FinalizedArrival:
    return FinalizedArrival(
        hotkey=f"miner-{index}",
        content_hash=_h(f"content:{index}:{block}"),
        url=f"https://example.invalid/{index}.tar.gz",
        block=block,
...
def test_lock_retry_is_exact_bounded_and_preserves_one_lease(
    ...
    def contended(*args, **kwargs):
        ...
        calls += 1
        if calls <= 2:
            raise IntakeError(operator._LOCK_COLLISION)
