"""Near-copy fingerprints + the submitted-delta copy comparator.

Pins the confirmed gaps: (1) a reformatted/recommented copy that flips the exact
content hash is still caught; (2) relocating a stolen body into an imported module
or padding the submission with extra variants does not evade containment demotion;
(3) two miners merely vendoring the same public utility never auto-demote. The
adversarial scenarios run end-to-end through ``fingerprint_submitted_delta`` +
``compare_submitted_deltas`` — the exact comparator chain intake uses
(``FinalizedIntakeStore.copy_predecessors``). The finalized-order / cross-hotkey
policy itself (later copy demoted, same hotkey exempt, retroactive demotion) is
tested at the intake layer in ``tests/test_chain_intake.py``.
"""
...
def test_structural_advisory_is_not_auto_demote(tmp_path):
    ...
    assert not decision.authoritative and decision.reason == "advisory_only"
    assert decision.shared_advisory     # the skeleton match is surfaced for review
...
def _write_variant_bundle(
    ...
    """Materialize explicit ``(slot, variant, source, entry[, dtypes])`` rows.

    ``dtypes`` (a list) is required when the bundle must pass the live intake
    projection (``fingerprint_submitted_delta``): variants of one slot must
    ...
    for slot, variant, source, entry, *rest in rows:
        ...
        if rest:
            lines.append("dtypes = [" + ", ".join(f'"{d}"' for d in rest[0]) + "]")
        lines.append("")
    (root / "manifest.toml").write_text("\n".join(lines))
    for rel, src in files.items():
        path = root / rel
...
def _containment_decision(earlier: Path, later: Path):
    return compare_submitted_deltas(
        fingerprint_submitted_delta(earlier), fingerprint_submitted_delta(later)
    )
...
def test_shared_vendored_utility_alone_is_not_a_copy(tmp_path):
    ...
    # demote; the shared fragments are surfaced as advisory only.
    util = ("import torch\n\ndef ceil_div(a, b):\n    return (a + b - 1) // b\n\n"
    ...
    assert not decision.authoritative and decision.reason == "advisory_only"
    assert decision.shared_fragments    # the vendored helper is visible, not demoting
...
'''
    ...
    multi = _write_variant_bundle(
        tmp_path / "multi",
        [
            (_SLOT, "stolen", "kernels/shared.py", "silu_and_mul", ["bfloat16"]),
            (_SLOT, "fresh", "kernels/shared.py", "silu_large", ["float16"]),
        ],
    ...
    decision = _containment_decision(singleton, multi)
    assert decision.authoritative and decision.reason == "symmetric_delta_containment"
...
'''
    multi = _write_variant_bundle(
        tmp_path / "multi",
        [
            (_SLOT, "stolen", "kernels/entry.py", "silu_and_mul", ["bfloat16"]),
            (_SLOT, "fresh", "kernels/entry.py", "silu_large", ["float16"]),
        ],
        {
            "kernels/entry.py": (
...
def test_stolen_singleton_is_contained_and_demoted_in_multi_variant_bundle(tmp_path):
    ...
    multi = _write_variant_bundle(
        tmp_path / "multi",
        [
            (_SLOT, "small", "kernels/small.py", "silu_and_mul", ["bfloat16"]),
            (_SLOT, "large", "kernels/large.py", "silu_and_mul", ["float16"]),
        ],
        _VARIANT_FILES,
    )
