...
def test_successful_pre_and_post_receipts_are_monotonic_and_query_exact_ids():
    ...
    before = guard.before_arm("candidate", deadline=clock.value + 20)
    after = guard.after_arm("candidate", deadline=clock.value + 20)
...
def test_malformed_gpu_query_fails_closed(gpu_output):
    ...
    with pytest.raises(DeviceStateParseError):
        _guard(policy, runner=runner, clock=clock).before_arm("baseline")
...
def test_compute_and_graphics_processes_reset_idle_consecutive_count():
    ...
    receipt = _guard(policy, runner=runner, clock=clock).before_arm("bookend")
...
def test_configuration_mutation_between_idle_samples_is_terminal():
    ...
    with pytest.raises(DeviceStateConfigurationError, match="power_limit_mw"):
        _guard(policy, runner=runner, clock=clock).before_arm("candidate")
...
def test_device_failure_types_expose_settlement_classification():
    assert DeviceStateConfigurationError("changed").validator_fault
    assert not DeviceStateConfigurationError("changed").retryable
    assert DeviceStateTimeoutError("busy").retryable
    assert not DeviceStateTimeoutError("busy").validator_fault
...
def test_absolute_deadline_is_forwarded_as_decreasing_subprocess_timeouts():
    ...
    _guard(policy, runner=runner, clock=clock).before_arm(
...
def test_active_observation_records_processes_without_treating_them_as_a_drain():
    policy = _policy()
    clock = FakeClock()
    active = "\n".join(
        (
    ...
    runner = ScriptedRunner([_gpu_output(policy)], [active], clock=clock)
    receipt = _guard(policy, runner=runner, clock=clock).observe(
        "candidate", "post-warmup", deadline=clock.value + 5
    )

    assert receipt.sequence == 1
    assert receipt.event == "post-warmup"
    assert not receipt.sample.idle
    assert [(item.pid, item.kind) for item in receipt.sample.processes] == [(444, "C")]
...
def test_active_conditioning_requires_consecutive_pinned_envelope_samples():
    ...
    receipt = _guard(policy, runner=runner, clock=clock).condition_active(
        "candidate", deadline=clock.value + 10
    )
...
def test_malformed_process_monitor_and_clock_regression_fail_closed():
    ...
    with pytest.raises(DeviceStateParseError):
        _guard(policy, runner=malformed, clock=clock).before_arm("baseline")

    healthy = ScriptedRunner(
        [_gpu_output(policy)], [_idle_process_output(policy)], clock=clock
    )
    guard = _guard(policy, runner=healthy, clock=clock)
    guard.before_arm("baseline")
    clock.value -= 100
    with pytest.raises(DeviceStateClockError):
        guard.after_arm("baseline")
