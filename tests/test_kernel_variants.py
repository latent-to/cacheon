...
from optima.competition import SLOT_MODE, resolve_competition
...
def _write_bundle(
    ...
    *,
    ...
    if competition:
        lines += ["[competition]", f'target = "{SLOT}"', 'mode = "slot"', ""]
...
def test_multiple_variants_remain_one_settlement_member(tmp_path):
    manifest = load_manifest(
        _write_bundle(
            tmp_path / "bundle",
            [{"variant": "h64"}, {"variant": "h128"}],
    ...
    assert resolved.target == SLOT
    assert resolved.members == (SLOT,)
    assert not resolved.crownable  # host-Python variants are system-lane products
