...
class TestPatchedForward7BTopology:
    ...
    def test_decode_output_shape(self, stub_policy):
        ...
        cfg = CacheConfig(
            num_layers=1, num_heads=NUM_HEADS, num_kv_heads=NUM_KV_HEADS,
            head_dim=HEAD_DIM, max_seq_len=256, dtype=torch.float32,
        )
        ...
        out, cache = fwd(hidden, position_ids=position_ids, position_embeddings=(cos, sin))
...
def stub_harness():
    ...
    # Tokenizer stub: returns fixed 5-token prompt for any input
    tok = MagicMock()
    tok.return_value = {"input_ids": torch.randint(0, 512, (1, 5))}
    tok.__call__ = lambda self, *a, **kw: {"input_ids": torch.randint(0, 512, (1, 5))}
    tok.eos_token_id = 2

    with patch("inference_engine.harness.AutoModelForCausalLM.from_pretrained", return_value=model), \
         patch("inference_engine.harness.AutoTokenizer.from_pretrained", return_value=tok):
...
class TestHarnessPatchUnpatch:
    ...
    def test_patch_replaces_all_forwards(self, stub_harness):
        originals = [
            ...
            for layer in stub_harness.model.model.layers
        ]
