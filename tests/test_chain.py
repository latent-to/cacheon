...
def test_storage_history_stops_saturated_pagination_at_competition_cursor():
    # Newest page is saturated (10 rows). Oldest entry is at/before the
...
def test_storage_history_keeps_paginating_while_oldest_is_after_cursor():
    # Twelve post-cursor reveals: the head page is saturated with 91-100, so
    # recovery must walk one older page to collect 89-90.
    history = tuple((block, f"payload-{block}") for block in range(89, 101))
    st = _MockSubtensor(hotkeys=["alice"], revealed={"alice": history}, block=200)
    pages: list[int] = []
    original = st.query_map
    ...
    st.query_map = query_map
    snapshot = chain.read_finalized_reveal_history(st, netuid=1, after_block=50)
    assert [row.block for row in snapshot.reveals] == list(range(89, 101))
    assert pages[0] == 200
    assert len(pages) >= 2
