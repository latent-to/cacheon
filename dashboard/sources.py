...
def load_sources(path, network, netuid, enrich):
    ...
    if (type(raw) is not dict or set(raw) not in ({"default", "sources"},
            {"default", "sources", "weight_producer_config"}) or not raw["sources"]):
        raise ValueError("dashboard sources require default and sources")
    if "weight_producer_config" in raw and not Path(raw["weight_producer_config"]).is_absolute():
        raise ValueError("weight producer config must be an absolute path")
...
def install_sources(app, api):
    ...
    app.state.dashboard_weight_producer_config = json.loads(Path(path).read_text()).get("weight_producer_config") if path else None
    ...
    def arenas():
        ...
        targets = _arena_targets(app.state.dashboard_weight_producer_config, app.state.dashboard_sources, block)
        for row in items:
