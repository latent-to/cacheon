...
elsewhere (cacheon's ``collective.ar_residual_rmsnorm`` slot / seam); the example miner
...
Usage (engine-agnostic):
    from cacheon_kernels.collective import fused_ar_rmsnorm as far
...
whatever build pipeline the host uses — cacheon's reviewed ``build_cuda_ext`` patcher,
