...
torch epilogue) lives in ``cacheon_kernels.override`` and is what runs + is tested on the
...
pinned flashinfer source. Precise vendoring spec (verified against flashinfer 0.6.13):
  ...
  ABI for the override device fn (matches cacheon_kernels.override / the EFC pattern, and
...
def run(x, topk_ids, topk_weights, prepared, out, *, epilogue_device=None, epilogue_torch=None):
    ...
    raise NotImplementedError(
        ...
        "cacheon_kernels.override._dense_moe with the torch epilogue."
    )
